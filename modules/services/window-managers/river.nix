...
let
  ...
  ] ++ (if cfg.systemd.runInService then [ "systemd-notify --ready" ] else [ ])
  ...
  options.wayland.windowManager.river = {
    ...
    systemd = {
      ...
      runInService = lib.mkEnableOption null // {
        default = false;
        description = ''
          Whether river should run inside sytemd (true) or outside of systemd (false).
          Running inside systemd means river lifecycle is fully known/managed by systemd.
          Stoping your computer or river crashing will stop the appropriates targets (graphical-session.target)
          And will make sure everything stays in sync.

          If river runs inside systemd, river logs will be available with journalctl.

          To start river, you will need to run systemctl --user start river and not run it from the command line for this to work.
        '';
      };
